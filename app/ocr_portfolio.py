...
def _fetch_current_price(symbol: str) -> float:
    """현재 시세 (native currency) — Yahoo Finance 무료. 실패 시 0.

    KR 6자리: .KS → .KQ 순서로 시도. USD 종목: 그대로.
    """
    sym = (symbol or "").strip()
    if not sym:
        return 0.0
    ...
    for ys in candidates:
        try:
            ...
            log.debug(f"_fetch_current_price {ys} 실패: {e}")
            continue
    return 0.0
...
3. **shares** — 보유 수량 (소수점 가능. 예 0.693214). 콤마/주 글자 제외. ⚠️ 화면에 "0.6932주" 같이 명시적으로 적힌 숫자만. **추측 절대 금지** — 안 보이면 반드시 null. (토스 메인 화면은 보통 수량 안 보임 → null 반환)
...
def extract_portfolio_from_image(image_bytes: bytes, krw_rate: float = 1380.0) -> list[dict]:
    ...
    for it in items:
        ...
        raw_shares = _clean_number(it.get("shares"))
        raw_current_value = _clean_number(it.get("current_value") or it.get("krw_invested"))
        raw_entry_price = _clean_number(it.get("entry_price"))
        ...
        if not currency:
            if is_kr_stock:
                currency = "KRW"
            elif raw_current_value > 100_000:
                currency = "KRW"  # 토스가 미국주식도 원화로 표시
            else:
                currency = "USD"

        # 입력 합리성
        if raw_current_value < 0 or raw_current_value > 10_000_000_000:
            log.warning(f"OCR 비정상 current_value ({sym}): {raw_current_value}")
            continue
        if raw_current_value <= 0 and raw_shares <= 0 and raw_entry_price <= 0:
        ...
        if currency == "KRW" and not is_kr_stock:
            ...
            current_value_native = raw_current_value

        # shares 결정 — 시세 알고 평가금액 알면 무조건 역산값 사용 (OCR보다 정확)
        final_shares = 0.0
        if current_price_native > 0 and current_value_native > 0:
            computed_shares = current_value_native / current_price_native
            if raw_shares <= 0:
                # OCR이 수량 못 읽음 → 역산값 사용
                final_shares = computed_shares
                log.info(f"{sym}: shares 역산 = {computed_shares:.6f} (값 {current_value_native:.2f} / 시세 {current_price_native:.2f})")
            else:
                # OCR이 수량 줬지만 검증
                ratio = raw_shares / computed_shares if computed_shares > 0 else 999
                if 0.7 <= ratio <= 1.5:  # 30% 오차 이내면 신뢰
                    final_shares = raw_shares
                else:
                    log.warning(f"{sym}: OCR shares={raw_shares} vs 역산={computed_shares:.4f} → 역산 사용 (OCR 오인식)")
                    final_shares = computed_shares
        elif raw_shares > 0:
            # 시세 못 받았는데 OCR shares는 있음 → 부득이 OCR 신뢰
            final_shares = raw_shares
            log.warning(f"{sym}: 시세 fetch 실패 → OCR shares={raw_shares} 사용 (검증 불가)")
        else:
            log.warning(f"{sym}: shares 결정 불가 (시세도 OCR도 없음) → 스킵")
            continue

        if final_shares <= 0 or final_shares > 10_000_000:
            log.warning(f"{sym}: 최종 shares 비정상 ({final_shares}) → 스킵")
            continue

        # entry_price 결정
        if raw_entry_price > 0:
            # OCR이 평단가 줬으면 native로 변환
            if currency == "KRW" and not is_kr_stock:
                entry_price = raw_entry_price / krw_rate
            else:
                entry_price = raw_entry_price
        elif current_price_native > 0:
            # 모르면 현재가로 (PnL 0%, 사용자가 ✏️로 수정 권장)
            entry_price = current_price_native
        elif current_value_native > 0 and final_shares > 0:
            # 마지막 수단: 평가금액 / 수량
            entry_price = current_value_native / final_shares
        else:
            log.warning(f"{sym}: entry_price 도출 실패 → 스킵")
            continue

        if entry_price <= 0:
            continue

        # krw_invested 계산 (통일 단위 KRW)
        krw_invested_total = (
            raw_current_value if currency == "KRW"
            else raw_current_value * krw_rate
        )
        if krw_invested_total <= 0:
            # 평가금액이 없으면 shares × entry_price × (USD면 환율) 로 추정
            krw_invested_total = final_shares * entry_price * (1 if is_kr_stock else krw_rate)

        seen_syms.add(sym)
        out.append({
            "symbol": sym,
            "name": name or sym,
            "shares": round(final_shares, 6),
