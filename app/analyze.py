...
# STRATEGY GUIDELINES (Intraday Day-Trading)
- **제1원칙 (24h Exit)**: 모든 포지션은 진입 후 24시간 이내(당일 또는 익일 장중) 청산을 목표로 한다.
- **적극적 단타**: 일일 변동성이 큰 지점에서 1.5%~3%의 짧은 수익 구간을 반복적으로 포착한다.
- **R-Multiple**: 초단타 특성상 1:1.2 이상의 손익비만 확보되면 진입 시그널을 생성한다.
- **수급 우선**: 장기 펀더멘털보다 현재 세션의 수급(Flow)과 모멘텀 돌파를 최우선한다.

# CORE RULES
1. **Day-Trade Only**: 1일 이내 매매 완료가 불가능해 보이는 무거운 흐름은 '관망' 처리.
2. **Volatility Focus**: 현재 가격이 주요 피벗(Pivot)이나 VWAP 대비 어떤 위치인지에 따라 즉각적인 대응가 제시.
3. **Specifics**: 목표가(Target)와 손절가(Stop)를 1일 변동성 범위 내로 타이트하게 제시.
