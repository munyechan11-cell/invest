...
| | 나가는 IP | 이 서비스에 |
|---|---|---|
| VPS (Vultr·Lightsail·Oracle) | 고정 | ✅ 권장 — `deploy/README.md` |
| Render Starter/Standard | 지역 공유 범위 | ❌ 토스가 거부합니다 |
| Render Pro + 전용 IP | 고정 3개 | 가능하지만 IP 세트당 월 $100 |

한국투자증권(KIS)은 IP 제한이 없어서 어디서든 됩니다. 토스를 쓰실 거면
**고정 IP VPS** 로 가세요 — 서울 리전 2GB 가 월 $12 이고, 자동매매처럼 상태를
들고 오래 도는 프로세스에는 원래 그쪽이 맞습니다.

설치 절차는 [`deploy/README.md`](deploy/README.md) 에 있습니다.

---

## 4. Render (IP 제한이 없는 증권사만)
