...
### 시작 전에 `validate` 가 말해 주는 것

사이징과 한도의 관계는 돌려보기 전에 산수로 나옵니다. 어긋나 있으면
`python -m quant validate` 가 숫자를 대고 말합니다 — 그러지 않으면 그 사실을
"왜 아무것도 안 사지" 로 하루 뒤에 알게 됩니다.

```
⚠ 신규 진입은 계좌 평가액이 약 175 ~ 1,053 USD 일 때만 나갑니다
  (한 종목이 최대 비중 30% 로 갈 때). 이 아래면 최소 주문금액에 못 미쳐
  건너뛰고, 위면 주문당 상한에 걸려 거절됩니다 — 둘 다 조용합니다
⚠ 하루 주문 건수 한도 10건은 걸리지 않습니다 — 거래대금 한도 500 을
  최소 주문 200 으로 나누면 최대 2건입니다      ← 이건 고쳐서 이제 안 뜹니다
```

두 번째 경고는 **한도끼리 모순**이라 출하 설정에서 없앴습니다:
`execution.min_order_notional` 을 `하루 거래대금 ÷ 하루 주문 건수` 로
끌어냅니다(미국 $50, 국내 5만원). 안전 천장 — `max_order_notional` 과 하루
한도 — 은 그대로 두고, **비용 통제인 최소 주문금액만** 내렸습니다. 올려서
맞추면 한도를 푸는 것이고 그건 다른 이야기입니다. 덤으로 진입 가능 구간이
1.5배에서 6배로 넓어져, 계좌가 조금 움직였다고 봇이 조용해지지 않습니다.

**실거래 사이징이 읽는 것은 `starting_cash` 가 아니라 증권사가 말하는 진짜
평가액입니다.** 그래서 `portfolio.max_position_weight`(전략의 언어)와
`broker.max_order_notional`(계좌의 언어)이 서로 모르는 채로 정해져 있으면,
잔고가 조금만 움직여도 모든 진입이 거절되거나 아무것도 사지 않습니다. 그
구간을 시작 전에 아는 것이 요점입니다.

봇도 같은 산수를 한 번 더 합니다 — 이번에는 **증권사에 붙은 뒤에, 진짜
평가액으로.** 벗어나 있으면 시작 배너와 함께 텔레그램으로 알리고, 화면의 안전
라벨이 "실행 중" 대신 **"실거래 실행 중 — 신규 진입 안 나감"** 이라고 씁니다.
붉은 오류색이 아니라 주황입니다: **손절과 청산은 그대로 나가기 때문입니다.**
아무것도 막지 않고 말만 합니다 — 이 검사가 새로운 정지 사유가 되면 고치려던
것보다 나쁩니다.

### 국내 주식의 호가단위 — `tick_ladder: krx`

KRX 의 호가단위는 **가격대마다 다릅니다**(2,000 / 5,000 / 20,000 / 50,000 /
200,000 / 500,000원이 경계). 고정 `tick_size` 하나만 적어 두면 그 종목이
경계를 넘는 순간 격자 밖 지정가가 되고 거래소가 거절합니다. 진입만 막히는
것이 아닙니다 — **손절도 지정가로 나갑니다.** 그래서 그 종목은 매 봉 거절만
반복하고 포지션에서 빠져나갈 길이 없어집니다.

```yaml
universe:
  symbols:
    - {ticker: "005930", venue: toss, quote_currency: KRW,
       tick_size: 100, lot_size: 1, tick_ladder: krx}
```

사다리를 켜면 주문 격자는 **그 가격에서의** 틱으로 계산됩니다. `tick_size` 는
비용 추정과 유니버스 필터가 쓰는 대표값으로 남습니다. 통화나 거래소 이름으로
추측하지 않고 설정에서 명시적으로 켭니다 — 고정 틱을 일부러 적어 둔 설정이
어느 날 조용히 다른 격자로 주문하기 시작하면, 그건 고쳐 준 게 아니라
바꿔치기입니다.
...
limits:
  ...
  timezone_offset_hours: 9       # 한국시간 자정 기준으로 초기화 (국내·코인)
                                 # 미국 전략은 -5 — 한국시간 자정은 뉴욕 개장
                                 # 30~90분 뒤라, 9 로 두면 한도가 장중에 새로 열립니다
```
...
## 성향이 다른 봇을 한 계좌에서 동시에 — 최대 4개

공격적 단기와 보수적 장기를 함께 보고 싶은데 계좌는 하나일 때. 자동매매 패널의
**여러 성향 동시 운용** 에서 에이전트를 추가하면 됩니다.

```
계좌 10만원 · 에이전트 2개
──────────────────────────────────────────────────────────
공격 · 단기    자본 70,000원   손절 10.0 ATR   하루주문 60건
보수 · 장기    자본 30,000원   손절  5.0 ATR   하루주문  5건
```

에이전트마다 **전략 설정·성향·손절·하루 한도가 전부 따로** 입니다. 같은 종목을
둘이 동시에, 각자의 손절로 들 수도 있습니다 — 공격형이 -25% 에서 버티는 동안
보수형은 -5% 에서 빠져나갑니다.

**자본은 나눕니다.** 비중 합이 100% 를 넘을 수 없고, 남는 몫은 현금으로 남습니다.
비중을 직접 적으면 나머지가 남은 몫을 나눠 갖고, 적어 둔 숫자는 다시 바뀌지
않습니다.

**모드도 에이전트마다.** 하나는 실거래, 하나는 관찰(dry_run)로 둘 수 있습니다 —
새 성향을 돈을 걸지 않고 같은 시세로 검증하는 방법입니다. 실거래 확인(전략 이름
타이핑)은 에이전트마다 따로 받습니다. 하나를 확인했다고 나머지가 열리면, 관찰용
으로 넣은 봇이 진짜 주문을 내고 있다는 사실을 모른 채로 하루를 보내게 됩니다.

### 계좌가 하나라는 것이 만드는 규칙

증권사는 005930 을 **합계 수량 하나** 로만 알려줍니다. "공격형의 10주" 라는 것은
이 시스템의 원장에만 있는 사실이고, 그래서 지켜야 하는 것들이 있습니다.

- **하루 한도가 두 겹입니다.** 에이전트별 한도 위에 **계좌 전체 한도** 가 따로
  있습니다. 합이 아닙니다 — 합으로 두면 넷이 각자 자기 한도를 지켜도 계좌는 네
  배를 잃습니다. 실거래 그룹은 계좌 한도 없이 시작할 수 없습니다.
- **한 에이전트는 자기 물량까지만 팝니다.** 자기 보유를 넘는 매도는 거절이 아니라
  자기 몫까지 줄여서 나갑니다 — 거절하면 손절이 막혀 손실 포지션에 갇힙니다.
- **원장과 계좌가 갈라지면 그룹 전체가 멈춥니다.** 어느 에이전트가 틀렸는지 알
  방법이 없기 때문입니다. 화면이 어느 종목이 얼마나 어긋났는지 말해 주므로,
  증권사 앱에서 확인한 뒤 다시 시작하면 됩니다.
- **수동 매매는 어느 에이전트인지 물어봅니다.** 전체 청산 확인창이 대상을 이름으로
  말하고 나머지는 그대로라고 알려 줍니다.
- **공매도는 켤 수 없습니다.** 한 에이전트의 공매도는 같은 종목을 든 다른
  에이전트의 보유를 계좌에서 없애는데, 그 사실을 원장이 표현하지 못합니다.

봇이 만들지 않은 보유 — 앱에서 직접 산 주식 — 는 **미귀속** 으로 따로 둡니다.
어느 에이전트도 팔지 않고 사이징에도 쓰이지 않지만, 증권사 합계에는 들어 있으므로
원장이 그것을 압니다.
