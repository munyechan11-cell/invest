...
| 아직 안 나간 주문 취소 | `POST /api/manual/cancel/{request_id}` |

접수한 주문은 즉시 나가지 않습니다. 대기열에 들어갔다가 엔진이 다음 봉을 처리할
때 발주됩니다. 그 사이에는 `GET /api/manual` 의 `pending` 에 그대로 떠 있고
(대시보드도 그 줄을 그립니다) 취소할 수 있습니다. 이미 발주된 뒤에 부르면
404 입니다 — 나간 주문을 무를 방법은 반대매매뿐이고, 그건 수수료와 세금이 붙는
새 거래라 서버가 대신 판단하지 않습니다.
