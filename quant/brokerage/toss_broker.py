...
class TossProvider(DataProvider):
    ...
    **왜 아직 웹소켓이 아닌가.** 토스는 실시간 체결·호가 스트리밍을
    `wss://openapi-ws.tossinvest.com/ws/v1` 로 제공한다고 REST 스펙 서두에
    적어 두었습니다. 그런데 그 소켓의 **프로토콜은 REST 스펙 안에 없습니다** —
    구독 선언, 인증 방식, 메시지 종류, keepalive, 구독 한도는 전부 별도의
    AsyncAPI 3.0 문서(`/openapi-docs/latest/asyncapi.json`)에 있고, 우리가 가진
    스펙 파일에는 그 문서의 **주소만** 들어 있습니다.

    그래서 여기서는 소켓을 열지 않습니다. 구독 메시지를 추측해서 적어 두면 그
    추측은 지금 이 파일의 `_FIELDS` 표가 한때 그랬던 것과 똑같은 방식으로
    틀립니다 — 다만 이번에는 "연결은 되는데 아무것도 안 오는" 모양이라
    호가창이 조용히 멈춘 것과 구분되지 않습니다. AsyncAPI 문서를 실제로 읽고
    나서 붙이세요. 붙일 자리는 `quant/data/feed.py` 의 `LiveBarFeed.admit()`
    입니다(확정 판정·중복 제거·못 본 구간 되메우기는 전송 방식과 무관합니다).

    그때까지 이 프로바이더는 **폴링**이고, `supports_streaming` 은 False 로
    둡니다. 이 값이 화면의 "실시간" 배지를 정하므로, 여기에 True 를 적는 순간
    배지가 거짓말을 시작합니다.
    """

    name = "toss"

    #: 호가·현재가는 실시간이지만 **봉은 마감돼야** 바뀝니다. 푸시 전송이
    #: 없다는 뜻이고, 위 docstring 이 그 이유입니다.
