...
class RealisticFillModel(FillModel):
    ...
    def __init__(self, max_volume_participation: float = 0.1,
                 ...
                 limit_volume_participation: float | None = None):
        self.participation = max_volume_participation
        self.requires_touch = limit_fill_requires_touch
        #: 지정가를 몇 틱 지나야 체결로 볼 것인가. 0 이면 예전처럼 스치기만
        #: 해도 체결됩니다.
        self.through_ticks = max(0, int(limit_through_ticks))
        #: 지정가 주문의 참여율. 시장가와 나눠 두는 이유는 아래
        #: `_volume_beyond` 주석에 있습니다.
        self.limit_participation = (max_volume_participation
                                    if limit_volume_participation is None
                                    else limit_volume_participation)

    def _traded_through(self, limit: float, side: OrderSide, symbol: Symbol,
                        bar: Bar) -> bool:
        """봉이 지정가를 `through_ticks` 만큼 지나갔는가.

        스치기(`bar.low == limit`)로는 부족합니다. 그 값이 봉의 극단이면 거기서
        오간 물량은 사실상 없고, 있었더라도 먼저 서 있던 주문들 몫입니다.
        백테스트에서 이걸 체결로 세면, 실거래에서는 안 걸릴 주문으로 돈을
        벌었다고 기록됩니다 — 지정가에 기대는 전략일수록 그 차이가 큽니다.

        `tick_size` 가 0 인 심볼 — 호가 단위 개념이 없는 합성 상품 — 에서는
        기준을 세울 수 없으니 옛 규칙대로 스치기로 되돌아갑니다.
        """
        edge = float(symbol.tick_size) * self.through_ticks
        # 부동소수점 여유. 70,000 - 100 같은 뺄셈이 마지막 자리에서 흔들려
        # 정확히 한 틱 지난 봉이 미체결로 떨어지는 것을 막습니다.
        tol = max(edge, abs(limit)) * 1e-12
        if side is OrderSide.BUY:
            return bar.low <= limit - edge + tol
        return bar.high >= limit + edge - tol

    def _volume_beyond(self, order: Order, bar: Bar) -> float:
        """지정가 너머에서 오간 물량의 추정치.

        봉 안의 거래량이 고가–저가에 고르게 퍼져 있다고 봅니다. 거친 가정이지만
        봉 전체 거래량을 쓰는 것보다 훨씬 덜 틀립니다 — 저가 근처에 걸어둔
        매수가 그 봉 거래량의 10% 를 통째로 받아간다는 가정이 사라집니다.

        `requires_touch=False` 는 "지정가를 낙관적으로 다 채운다" 는 탈출구
        입니다. 그 모드에서까지 이 추정을 적용하면, 봉 범위 밖의 지정가가
        지분 0 으로 잘려 **1주만** 체결됩니다 — 낙관과 정반대의 결과이고,
        그 플래그를 쓰던 쪽은 이유도 모르고 물량을 잃습니다.
        """
        limit = order.limit_price
        if limit is None or bar.range <= 0 or not self.requires_touch:
            return bar.volume
        span = (limit - bar.low) if order.side is OrderSide.BUY else (bar.high - limit)
        # `_traded_through` 와 같은 이유의 여유. 없으면 (99.05-99.0) 이
        # 0.04999999999999716 로 나와서 lot 격자에서 한 단위를 잃습니다.
        span += bar.range * 1e-12
        return bar.volume * min(max(span / bar.range, 0.0), 1.0)

    def max_fillable(self, order, bar):
        if order.type in (OrderType.LIMIT, OrderType.STOP_LIMIT):
            available, rate = self._volume_beyond(order, bar), self.limit_participation
        else:
            available, rate = bar.volume, self.participation
        if rate <= 0 or bar.volume <= 0:
            return order.remaining
        cap = Decimal(str(available * rate))
    ...
    def fill_price(self, order, bar, quote, slippage):
        ...
        if order.type is OrderType.LIMIT:
            ...
            if self.requires_touch and not self._traded_through(
                    limit, order.side, order.symbol, bar):
                return None, False
        ...
        if order.type in (OrderType.STOP, OrderType.STOP_LIMIT):
            ...
            # 스톱리밋의 지정가도 같은 규칙을 받습니다. 스톱이 걸린 봉의 극단이
            # 마침 지정가면, 그건 체결됐다는 근거가 아니라 우연입니다.
            if self.requires_touch and not self._traded_through(
                    limit, order.side, order.symbol, bar):
                return None, False
            return limit, True
